
/* This file is generated by glib-mkenums, do not modify it. This code is licensed under the same license as the containing project. Note that it links to GLib, so must comply with the LGPL linking clauses. */

#ifndef __MUFFIN_ENUM_TYPES_H__
#define __MUFFIN_ENUM_TYPES_H__

#include <glib-object.h>

G_BEGIN_DECLS
/* enumerations from "meta/boxes.h" */
GType meta_edge_type_get_type (void) G_GNUC_CONST;
#define META_TYPE_EDGE_TYPE (meta_edge_type_get_type())
/* enumerations from "meta/common.h" */
GType meta_frame_flags_get_type (void) G_GNUC_CONST;
#define META_TYPE_FRAME_FLAGS (meta_frame_flags_get_type())
GType meta_menu_op_get_type (void) G_GNUC_CONST;
#define META_TYPE_MENU_OP (meta_menu_op_get_type())
GType meta_grab_op_get_type (void) G_GNUC_CONST;
#define META_TYPE_GRAB_OP (meta_grab_op_get_type())
GType meta_cursor_get_type (void) G_GNUC_CONST;
#define META_TYPE_CURSOR (meta_cursor_get_type())
GType meta_frame_type_get_type (void) G_GNUC_CONST;
#define META_TYPE_FRAME_TYPE (meta_frame_type_get_type())
GType meta_virtual_modifier_get_type (void) G_GNUC_CONST;
#define META_TYPE_VIRTUAL_MODIFIER (meta_virtual_modifier_get_type())
GType meta_direction_get_type (void) G_GNUC_CONST;
#define META_TYPE_DIRECTION (meta_direction_get_type())
GType meta_motion_direction_get_type (void) G_GNUC_CONST;
#define META_TYPE_MOTION_DIRECTION (meta_motion_direction_get_type())
GType meta_side_get_type (void) G_GNUC_CONST;
#define META_TYPE_SIDE (meta_side_get_type())
GType meta_button_function_get_type (void) G_GNUC_CONST;
#define META_TYPE_BUTTON_FUNCTION (meta_button_function_get_type())
GType meta_tile_mode_get_type (void) G_GNUC_CONST;
#define META_TYPE_TILE_MODE (meta_tile_mode_get_type())
GType meta_window_tile_type_get_type (void) G_GNUC_CONST;
#define META_TYPE_WINDOW_TILE_TYPE (meta_window_tile_type_get_type())
GType meta_bell_type_get_type (void) G_GNUC_CONST;
#define META_TYPE_BELL_TYPE (meta_bell_type_get_type())
GType meta_stack_layer_get_type (void) G_GNUC_CONST;
#define META_TYPE_STACK_LAYER (meta_stack_layer_get_type())
GType meta_placement_mode_get_type (void) G_GNUC_CONST;
#define META_TYPE_PLACEMENT_MODE (meta_placement_mode_get_type())
GType meta_background_transition_get_type (void) G_GNUC_CONST;
#define META_TYPE_BACKGROUND_TRANSITION (meta_background_transition_get_type())
GType meta_sync_method_get_type (void) G_GNUC_CONST;
#define META_TYPE_SYNC_METHOD (meta_sync_method_get_type())
/* enumerations from "meta/compositor.h" */
GType meta_comp_effect_get_type (void) G_GNUC_CONST;
#define META_TYPE_COMP_EFFECT (meta_comp_effect_get_type())
/* enumerations from "meta/display.h" */
GType meta_tab_list_get_type (void) G_GNUC_CONST;
#define META_TYPE_TAB_LIST (meta_tab_list_get_type())
GType meta_tab_show_type_get_type (void) G_GNUC_CONST;
#define META_TYPE_TAB_SHOW_TYPE (meta_tab_show_type_get_type())
GType meta_list_windows_flags_get_type (void) G_GNUC_CONST;
#define META_TYPE_LIST_WINDOWS_FLAGS (meta_list_windows_flags_get_type())
/* enumerations from "meta/gradient.h" */
GType meta_gradient_type_get_type (void) G_GNUC_CONST;
#define META_TYPE_GRADIENT_TYPE (meta_gradient_type_get_type())
/* enumerations from "meta/main.h" */
GType meta_exit_code_get_type (void) G_GNUC_CONST;
#define META_TYPE_EXIT_CODE (meta_exit_code_get_type())
/* enumerations from "meta/meta-plugin.h" */
GType meta_modal_options_get_type (void) G_GNUC_CONST;
#define META_TYPE_MODAL_OPTIONS (meta_modal_options_get_type())
/* enumerations from "meta/prefs.h" */
GType meta_preference_get_type (void) G_GNUC_CONST;
#define META_TYPE_PREFERENCE (meta_preference_get_type())
GType meta_key_binding_action_get_type (void) G_GNUC_CONST;
#define META_TYPE_KEY_BINDING_ACTION (meta_key_binding_action_get_type())
GType meta_key_binding_flags_get_type (void) G_GNUC_CONST;
#define META_TYPE_KEY_BINDING_FLAGS (meta_key_binding_flags_get_type())
/* enumerations from "meta/screen.h" */
GType meta_screen_corner_get_type (void) G_GNUC_CONST;
#define META_TYPE_SCREEN_CORNER (meta_screen_corner_get_type())
/* enumerations from "meta/util.h" */
GType meta_debug_topic_get_type (void) G_GNUC_CONST;
#define META_TYPE_DEBUG_TOPIC (meta_debug_topic_get_type())
GType meta_later_type_get_type (void) G_GNUC_CONST;
#define META_TYPE_LATER_TYPE (meta_later_type_get_type())
/* enumerations from "meta/window.h" */
GType meta_window_type_get_type (void) G_GNUC_CONST;
#define META_TYPE_WINDOW_TYPE (meta_window_type_get_type())
GType meta_maximize_flags_get_type (void) G_GNUC_CONST;
#define META_TYPE_MAXIMIZE_FLAGS (meta_maximize_flags_get_type())
G_END_DECLS

#endif /* !__MUFFIN_ENUM_TYPES_H__ */

/* Generated data ends here */

